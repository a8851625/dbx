...
<Tabs groupId="dev-platform" items={['macOS', 'Linux', 'Windows']}>
  ...
  <Tab value="Windows">
    Windows 通常不需要额外安装 Web 版 DBX 的系统依赖。
  </Tab>
</Tabs>
...
如果希望连 PostgreSQL 一起本地拉起：

```bash
docker compose -f deploy/docker-compose.yml up --build
```

这个 compose 栈现在也会同时拉起本地 Keycloak，用来做真实 OIDC 联调：
...
- 测试账号: `dbx-admin / dbx-admin-123`（`admin@example.com`）

可以用下面的命令做一次完整的认证 smoke：

```bash
python3 deploy/scripts/oidc_compose_smoke.py
```

shared test 所需环境变量和 QA handoff 清单见 `backend/docs/oidc-shared-test.md`。
